...
This is a mode where we are testing maximum Suricata throughput and trying to converge on a defined packet drop rate. The search returns the hightest found multiplier that is under or equal the specified allowed drop rate. If none is found, the run fails (raises `MultiplierNotFoundError`).
