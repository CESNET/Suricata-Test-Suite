...
from typing import Dict, List, Literal, Self, Sequence, Tuple
...
class BaseTrexClientManager:
    ...
    def is_mode_allowed(self, mode: TrexMode) -> bool:
        """
        Disallow some modes for a Profile
        """
        return True
    ...
    def __init__(
        ...
        if not self.is_mode_allowed(mode):
            raise ValueError(f"mode {mode} is not allowed for {self}")

        if len(self.pcaps) < 1:
            raise ValueError("pcaps must contain at least one pcap")
        ...
        match self.mode:
            ...
            case TrexMode.STF:
                ...
                assert isinstance(trex_gen, List)
                assert isinstance(trex_gen[0], str)
